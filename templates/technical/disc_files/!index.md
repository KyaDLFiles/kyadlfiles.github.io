...
|Number|Type|Description|Loaded partial sectors|Shell elevators|Picture|
|-|-|-|-|-|-|
...
**Always loaded elements**: Slide going from upper to lower level (broken), tree going from lower to upper section (broken)
...
|7|!{!2}Partial|Last section of freefall and initial part of the outside (shell elevator is part of sector 3), connecting sectors 2 and 3|||
...
This is further supported by the fact that sector 3 calls for sector 9 to be loaded, despite it being completely empty, which suggests that sector 9 might've also contained a connection between the outer area and the scrapped area  

This may seem unusual as sector 4 does not call for sector 9 to be loaded, but that could be explained in several ways: 
...
Ultimately, given this last point, and without having any access to later prototype builds which still contain the scrapped section (and without any input from the developers), this theory can neither be confirmed nor denied  

Note that both sectors have last modified timestamps in line with other sectors, with the date being the same as the timestamp of the game executable, so no meaningful info can be extrapolated from those
