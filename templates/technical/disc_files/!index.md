...
- sector 9 connected to sector 4 through some sort of one-way path/passage, which means there wouldn't have been a need to keep sector 9 loaded as it was impossible to go back to it or even see it once inside (this is supported by the fact that in the May 12 beta, the entrance to the scrapped section is one-way, and the whole section needs to be traversed to get out of it).  
